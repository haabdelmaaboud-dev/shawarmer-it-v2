...
const DashboardView = {
  render() {
    ...
    const chartSection = this._renderCharts(stores);
    const kpiSection = `
      <div class="kpi-grid">
        ${UI.kpiCard({
    ...
    const progressSection = isEng ? this._engineerProgress(stats) : this._teamOverview(stores);
  ...
  _renderCharts(stores) {
    const byArea = {};
    stores.forEach(s => {
      if (!byArea[s.area]) byArea[s.area] = { t: 0, h: 0, w: 0, c: 0 };
      byArea[s.area].t++;
      const health = STATE.healthLabel(s);
      if (health === 'healthy') byArea[s.area].h++;
      else if (health === 'warning') byArea[s.area].w++;
      else byArea[s.area].c++;
    });

    const areas = Object.entries(byArea).sort((a, b) => b[1].t - a[1].t).slice(0, 6);
    const maxVal = Math.max(...areas.map(a => a[1].t));

    return `
      <div class="card">
        <div class="card-header">
          <h3><i class="ti ti-chart-bar"></i> Stores by Area</h3>
        </div>
        <div class="card-body">
          <div class="chart-container">
            ${areas.map(([name, data]) => `
              <div class="chart-bar">
                <div class="chart-bar-value">${data.t}</div>
                <div class="chart-bar-fill chart-healthy" style="height:${(data.t/maxVal*100)}%"></div>
